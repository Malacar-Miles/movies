...
export const addMovieToDatabase = async (movie: Movie) => {
  try {
    ...
    if (error instanceof Error)
      console.log(
        "Failed to upload specific movie data to DB: ",
        error.message
      );
  }
};
...
export const getMoviesByCategoryFromDatabase = async (
  categoryId: string,
  itemId: string
) => {
  try {
    ...
    switch (categoryId) {
      case categories.genre:
        q = query(
          moviesCollection,
          where("genre", "array-contains", mapIdToGenre[itemId])
        );
        break;
      case categories.country:
        q = query(
          moviesCollection,
          where("country", "array-contains", toTitleCase(itemId))
        );
        break;
      case categories.subtitles:
        q = query(
          ...
          where(
            "subs",
            "array-contains",
            mapLanguageToCode[toTitleCase(itemId)]
          )
        );
        break;
      case categories.decade:
        ...
        q = query(
          moviesCollection,
          where("year", ">=", years.startingYear),
          where("year", "<=", years.endingYear)
        );
        break;
      default:
    ...
    if (error instanceof Error)
      console.log(
        "Failed to fetch filtered movie data from DB: ",
        error.message
      );
    return null;
  }
...
export const searchAllMoviesFromDatabase = async (searchQuery: string) => {
  ...
  try {
    ...
    if (error instanceof Error)
      console.log(
        "Failed to fetch all movie data from DB for searching: ",
        error.message
      );
    return null;
  }
...
// Get a movie document with a specific id from Firestore,
// return it as a movie object and update the viewcount
export const getMovieFromDatabase = async (movieId: string) => {
  try {
    ...
    if (error instanceof Error)
      console.log(
        "Failed to fetch specific movie data from DB: ",
        error.message
      );
    return null;
  }
